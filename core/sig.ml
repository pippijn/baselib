...
let writable_of_sexp (_ : Sexplib.Sexp.t) : writable = Writable
let sexp_of_writable (_ : writable) = Sexplib.Sexp.List []
let readonly_of_sexp (_ : Sexplib.Sexp.t) : readonly = Readonly
let sexp_of_readonly (_ : readonly) = Sexplib.Sexp.List []
...
module type DenseIntMapType = sig
  type integer = private int
  type ('a, 'mutability) t

  val t_of_sexp : (Sexplib.Sexp.t -> 'a) -> (Sexplib.Sexp.t -> 'm) -> Sexplib.Sexp.t -> ('a, 'm) t
  val sexp_of_t : ('a -> Sexplib.Sexp.t) -> ('m -> Sexplib.Sexp.t) -> ('a, 'm) t -> Sexplib.Sexp.t
